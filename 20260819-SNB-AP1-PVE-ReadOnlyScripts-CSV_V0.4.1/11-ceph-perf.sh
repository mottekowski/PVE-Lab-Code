...
if run_ready "$SCOPE" "$CATEGORY" "Daemon Perf Dump" "ceph tell osd.* perf dump | head -n 80"; then
    ...
    if [[ -s "$PERF_NORMALIZED" ]]; then
        while IFS=$'\t' read -r osd path value; do
            emit_record "$osd" "$CATEGORY" "Daemon Perf / $path" "$value" "ceph tell osd.* perf dump | head -n 80" "$RUN_TS" "Read-only 80-line sample; full perf dump in Evidence=${RUN_EVIDENCE_REL}"
        done < "$PERF_NORMALIZED"
    else
        emit_status "$SCOPE" "$CATEGORY" "Daemon Perf Dump" "NO_NORMALIZED_COUNTERS" "ceph tell osd.* perf dump | head -n 80" "$RUN_TS" "Command succeeded, but the first 80 raw lines contained no scalar counters matching the generic normalizer; full output retained in Evidence=${RUN_EVIDENCE_REL}"
    fi
fi
