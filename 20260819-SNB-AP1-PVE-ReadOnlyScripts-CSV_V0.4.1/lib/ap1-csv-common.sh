#!/usr/bin/env bash
# Shared CSV/evidence functions for AP1 V0.4.1 collectors.
# Normalization principle: one CSV row = one atomic parameter/value pair.
# V0.4.1 executes read-only commands locally or through SSH while all CSV/evidence files remain on the coordinator.
