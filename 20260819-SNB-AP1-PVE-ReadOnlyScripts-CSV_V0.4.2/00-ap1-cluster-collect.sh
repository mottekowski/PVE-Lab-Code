#!/usr/bin/env bash
# AP1 V0.4.2 central cluster collector.
...
Options:
  ...
  --timezone ZONE           IANA timezone for Run-ID and CSV timestamps. Default: Europe/Berlin.
...
Environment:
  AP1_TIMEZONE             IANA timezone for Run-ID/timestamps (default Europe/Berlin).
...
if [[ ! -e "/usr/share/zoneinfo/${AP1_TIMEZONE}" ]]; then
    printf 'Invalid --timezone/AP1_TIMEZONE: %s (zoneinfo file not found)\n' "$AP1_TIMEZONE" >&2
    exit 64
fi
export AP1_TIMEZONE TZ="$AP1_TIMEZONE"
if [[ -z "$RUN_ID" ]]; then
...
run_collector() {
    ...
    AP1_TARGET_HOST="$node" AP1_RUN_ID="$RUN_ID" AP1_SSH_USER="$AP1_SSH_USER" \
      ...
      AP1_SSH_CONNECT_TIMEOUT="${AP1_SSH_CONNECT_TIMEOUT:-10}" AP1_SSH_OPTIONS="${AP1_SSH_OPTIONS:-}" AP1_TIMEZONE="$AP1_TIMEZONE" \
