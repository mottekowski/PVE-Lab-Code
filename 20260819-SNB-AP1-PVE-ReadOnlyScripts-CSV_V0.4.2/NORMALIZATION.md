# Normalisierungsprinzip - V0.4.2

V0.4.2 übernimmt das fachliche Normalisierungsprinzip sowie die Multi-Host-Erfassung aus V0.4.1 unverändert und ergänzt ausschließlich die dokumentierten Quality-/Usability-Korrekturen.
...
Das gemeinsame `YYYYMMDD_HHMM` bildet eine Collection-Run-ID und erlaubt, alle Dateien eines Kundentermins eindeutig zusammenzufassen. Die automatische Run-ID wird in `AP1_TIMEZONE` erzeugt (Default `Europe/Berlin`). Das CSV-Feld `Zeitstempel` ist ISO 8601 mit explizitem UTC-Offset, sodass keine implizite UTC/Local-Time-Konvertierung erforderlich ist.
