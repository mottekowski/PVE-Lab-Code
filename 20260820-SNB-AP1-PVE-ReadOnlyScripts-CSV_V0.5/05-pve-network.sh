...
if [[ $RUN_RC -eq 127 ]]; then
    emit_status "$HOST_SHORT" "LLDP" "Neighbors" "OPTIONAL_TOOL_NOT_INSTALLED" "lldpcli show neighbors" "$RUN_TS" "Optional tool lldpcli is not installed on target host; no LLDP data collected; Evidence=${RUN_EVIDENCE_REL}"
elif [[ $RUN_RC -ne 0 ]]; then
    run_ready "$HOST_SHORT" "LLDP" "Neighbors" "lldpcli show neighbors" || true
